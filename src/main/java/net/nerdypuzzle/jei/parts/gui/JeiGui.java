package net.nerdypuzzle.jei.parts.gui;
...
public class JeiGui extends JComponent implements MouseMotionListener, MouseListener, IZoomable {
    ...
    private final JeiGuiEditor JeiGuiEditor;
    public WYSIWYGEditor fakeEditor;

    private final Image background = UIRES.get("jeiguieditor").getImage();
    ...
    void removeMode() {
        JeiGuiEditor.components.remove(selected);
        selected = null;
        repaint();
    }

    public void addComponent(GUIComponent component) {
        JeiGuiEditor.components.add(component);
        repaint();
    }
    ...
    @Override public void paint(Graphics gx) {
        ...
        if (selected != null) {
            ...
            int cw = selected.getWidth(JeiGuiEditor.mcreator.getWorkspace());
            int ch = selected.getHeight(JeiGuiEditor.mcreator.getWorkspace());
        ...
        if (componentMoveMode) {
            ...
            if (positioningModeSettingWidth || positioningModeSettingHeight) {
                ...
                g.drawLine(0, oy * 2 + selected.getHeight(JeiGuiEditor.mcreator.getWorkspace()) * 2, getWidth(),
                        oy * 2 + selected.getHeight(JeiGuiEditor.mcreator.getWorkspace()) * 2);
                g.drawLine(ox * 2 + selected.getWidth(JeiGuiEditor.mcreator.getWorkspace()) * 2, 0,
                        ox * 2 + selected.getWidth(JeiGuiEditor.mcreator.getWorkspace()) * 2, getHeight());
            }
        }
    ...
    private void drawJeiGui(Graphics2D g) {
        ...
        int gw = (Integer) JeiGuiEditor.spa1.getValue();
        int gh = (Integer) JeiGuiEditor.spa2.getValue();

        if (JeiGuiEditor.renderBgLayer.isSelected()) {
            g.drawImage(JeiGuiEditor.guiImage, (int) Math.ceil(W / 2.0 - gw / 2.0),
                    (int) Math.ceil(H / 2.0 - gh / 2.0), gw, gh, this);
        }

        List<GUIComponent> toDraw = new ArrayList<>(JeiGuiEditor.components);
        ...
        toDraw.stream().sorted().forEach(component -> {
            ...
            if (fakeEditor == null) {
                fakeEditor = JeiGuiEditor.getFakeEditor();
            }
        ...
        if (componentMoveMode) {
            ...
            if (component != null) {
                if (e.getClickCount() > 1) {
                    JeiGuiEditor.editCurrentlySelectedComponent();
                } else {
                    JeiGuiEditor.list.setSelectedValue(component, true);
                    JeiGuiEditor.list.requestFocus();
                }
            } else {
                JeiGuiEditor.list.clearSelection();
                this.selected = null;
                repaint();
            }
        }
    ...
    @Nullable private GUIComponent getGUIComponentAt(int ex, int ey) {
        List<GUIComponent> guiComponentList = new ArrayList<>(JeiGuiEditor.components);
        ...
        for (GUIComponent component : guiComponentList) {
            if (ex >= component.getX() && ex <= component.getX() + component.getWidth(
                    JeiGuiEditor.mcreator.getWorkspace())) {
    ...
    private void finishGUIComponentMove() {
        for (int i = 0; i < JeiGuiEditor.components.size(); i++) {
            GUIComponent component = JeiGuiEditor.components.get(i);
